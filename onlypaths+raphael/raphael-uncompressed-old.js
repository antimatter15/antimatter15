    ...
    return (function (r, args) {
        ...
        if (r.type == "VML") {
            ...
            var thePath = function (params, pathString, SVG) {
                ...
                p.curveTo = function () {
                    ...
                    if (arguments.length == 6) {
                        ...
                        this.last.x = p._getX(/*Math.round*/(parseFloat(arguments[4], 10) * 100) / 100);
                        this.last.y = p._getY(/*Math.round*/(parseFloat(arguments[5], 10) * 100) / 100);
                        this.last.bx = p._getX(/*Math.round*/(parseFloat(arguments[2], 10) * 100) / 100);
                        this.last.by = p._getY(/*Math.round*/(parseFloat(arguments[3], 10) * 100) / 100);
                    } else {
                        ...
                        this.last.x = p._getX(/*Math.round*/(parseFloat(arguments[2], 10) * 100) / 100);
                        this.last.y = p._getY(/*Math.round*/(parseFloat(arguments[3], 10) * 100) / 100);
                        this.last.bx = p._getX(/*Math.round*/(parseFloat(arguments[0], 10) * 100) / 100);
                        this.last.by = p._getY(/*Math.round*/(parseFloat(arguments[1], 10) * 100) / 100);
                    }
                    var oldD = this[0].getAttribute("d") || "";
                    this[0].setAttribute("d", oldD + d);
            ...
            r._create = function () {
                ...
                container = {canvas: C.canvas};
                C.defs = document.createElementNS(C.svgns, "defs");
                C.gradients = 0;
                C.canvas.appendChild(C.defs);
                for (var prop in C) {
                    if (prop != "create") {
                        container[prop] = C[prop];
                    }
        ...
        C.safari = function () {
            if (this.type == "SVG") {
                ...
                setTimeout(function () {rect.remove();}, 1);
            }
        };
        Raphael = function () {
